...
- 到边界时，再做 `report()` / `snapshot()` / `exposure(...)`
...
StructError<RepoReason>
  ->.source_err() + upcast() (错误进入 + 跨层转换)
StructError<ServiceReason>
  -> report() / snapshot().stable_export() / exposure(...)
```
...
**3. ~~`wrap_as(reason, detail)`~~** — 已废弃，使用 `source_err` 替代。
`source_err` 现在统一处理 `std::error::Error` 和 `StructError` 两种输入。
