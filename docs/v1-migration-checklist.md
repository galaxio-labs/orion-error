...
本文档记录 `orion-error` 从 `0.6.2` 演进到 `0.6.3` 时，`V1 API` 的迁移主路径。

这里的 `V1` 指的是：

- 兼容版收敛
- 不是 V2 破坏性重构
- 不假装解决 `StructError: StdError` 的根问题
...
- 上下文主命名使用 `at(...)` / `doing(...)`
  ...
  - `at(...)` 在 V1 不引入新的 `TargetRef` / path kind / location kind 模型
  - `doing(...)` 在 V1 中只是 `want(...)` 的命名糖衣
- `into_as(...)` 不对 `E: StdError` 做 blanket impl
- 只有在需要显式声明“这是一个本地 raw `StdError`，并且显式实现了 `RawStdError`”时，才使用 `raw_source(...)`
  - 不要重新引入 `E: StdError` blanket 风格入口
  - 不要试图让 `StructError<_>` 走 `raw_source(...)`
- `wrap_as(...)` 是 V1 的结构化错误上卷主路径
  - `wrap_as(...)` 长期应作为独立 trait 的主命名
  - 即便实现层暂时复用旧逻辑，也不要把它和 `err_wrap(...)` 混成同一套主叙事
...
- `raw_source(...)`
  - 它只表示“显式 raw StdError 包装”
  - 不是把任意 `Display` 值塞进 source 链的通用入口

## 旧代码迁移说明

这一节用于回答一个更具体的问题：

> 看到旧代码里全是 `owe(...)` / `owe_*()` / `owe_*_source()` / `err_wrap(...)` / `want(...)` / `with_source(...)`，现在到底应该怎么改？
...
- `into_as(...)` 在 V1 只对封闭的 `UnstructuredSource` 开放
- `StructError<_>` 不能走 `into_as(...)`
...
- `raw_source(...)` 的目标是给 `into_as(...)` 提供显式 opt-in
- 不要把它扩展到 `Display` only 值
- 不要试图让 `StructError<_>` 走这条路
- 如果一个第三方 raw error 已被官方 feature 集成，就不需要 `raw_source(...)`
...
- 类似 `wrap_conf(...)` 的便捷版，如果存在，也只适合少数有固定顶层 reason 的项目层封装
- 不应把 `wrap_conf(...)` 一类糖衣当成通用库的唯一主路径
...
- `with_struct_source(...)` 只用于你明确想把下层 `StructError<_>` 作为结构化 source 直接挂上去
- 如果你的语义是“上卷建立新边界”，应优先使用 `wrap_as(...)`
...
V1 对第三方错误的集成建议采用 feature gating，而不是默认全开。
...
- `anyhow`
  ...
  - 默认视为“已聚合但未结构化错误”
  - 不做泛化的链式魔法扫描
  - 如未来支持提取 `orion-error` 官方桥接类型，应作为专门规则处理
- `serde_json`
  - 如果提供 `serde_json::Error -> into_as(...)` 集成，则应是 feature 控制的可选运行时依赖
  - 如果只是测试构造样例错误，则应放在 `dev-dependencies`
- `toml`
  - `toml::de::Error` / `toml::ser::Error` 同样应通过 feature 控制

不建议：

- 因为测试方便，把第三方 error 直接拉成默认运行时依赖
- 文档声称已支持某类第三方错误，但实现只存在于测试代码里

## V1 明确不承诺的能力

为避免文档超前于实现，`0.6.3` 这轮 V1 迁移不承诺以下能力：

- `at(...)` 自动把 `&Path` / `&PathBuf` / `&str` 提升为结构化 target kind
- `doing(...)` 改写 `OperationContext` 的底层模型
- `into_as(...)` 自动识别任意 `StdError`
- `raw_source(...)` 接受任意 `Display` only 值
- `wrap_as(...)` 与 `err_wrap(...)` 在长期 API 上共用同一套主叙事

一句话说，V1 先收敛“正确用法”，不假装已经完成“终局模型”。
...
如果 review 时拿不准某段旧代码该迁到哪条路径，至少先问 4 个问题：

1. 上游错误是不是第一次进入结构化体系？
2. 上游是不是已经是 `StructError<_>`？
3. 当前附加的是普通 `StdError` source，还是结构化 source？
4. 当前是在建立新边界，还是只做 reason 转换？

通常对应关系就是：

- 第一次结构化：`into_as(...)`
- 已结构化上卷：`wrap_as(...)`
- 已结构化仅 reason 转换：`err_conv()`
