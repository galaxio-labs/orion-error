...
root 不承诺重新暴露 reason trait、protocol type、report type、
...
- `conversion`
  主路径转换 trait：`SourceErr`、`ErrorWith`、`ConvErr`、
...
- `serde_json`
  开启 protocol JSON projection 方法：
  `to_http_error_json()`、`to_cli_error_json()`、
