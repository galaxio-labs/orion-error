...
**gRPC：稳定错误分类。** gRPC 将跨语言 RPC 失败收敛为一组标准状态码，例如参数无效、资源不存在、依赖不可用、超时、内部错误。调用方不需要解析错误文本，就可以基于稳定分类做重试、降级、告警和用户响应。

**PostgreSQL：错误码优先于错误文本。** PostgreSQL 使用稳定的 SQLSTATE 错误码表达错误条件。应用程序应依赖错误码判断失败类型，而不是依赖可能变化或本地化的错误消息。这体现了机器契约和人类文案的分离。

**Kubernetes：错误也是系统状态。** Kubernetes 不只在请求失败时返回错误，还把对象是否就绪、是否被接受、为什么失败等信息长期写入 `status` 和 condition。错误不只是一次返回值，而是可查询、可观察、可自动化处理的系统状态。

**Terraform：面向操作者的诊断。** Terraform 的 diagnostics 不只是报告"失败"，还会区分错误和警告，指出配置路径，给出摘要和细节。它关注的是让操作者知道哪里错、为什么错、下一步该怎么修。

**rustc：诊断信息是产品体验。** Rust 编译器的错误包含错误码、源码位置、主次标注、note 和 help。它把错误处理做成开发体验的一部分，而不是只给出编译失败的字符串。

**Envoy：失败原因进入观测系统。** Envoy 在访问日志中使用稳定的 response flags 表达无健康 upstream、连接失败、无路由、上游溢出等失败原因。这些短码让边界层错误可以被聚合、搜索、告警和自动分析。
...
下面这张图表达一个错误从底层失败进入结构化体系、跨语义域传播、最终在边界投影的过程：
...
图中的关键点是：错误在内部传播时不是被压扁成字符串，而是同时携带治理通道和诊断通道。边界层根据策略选择不同视图，而不是重新解释错误。
...
WarpParse 的错误处理不是单点返回错误，而是覆盖规则开发、规则验证、运行时解析、管线执行、边界输出和运维观测的一条完整链路。
