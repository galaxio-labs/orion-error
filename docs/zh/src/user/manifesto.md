...
- **工程落地层**：Rust 下如何用 `orion-error` 实现稳定身份、诊断链、边界输出和桥接。
...
泛型擦除，异常机制主导。但 cause chain 机制成熟，Spring 的 `@ControllerAdvice`、filter、interceptor 已是集中策略的常见模式。Java 17+ 的 sealed class、record 和 pattern matching 让有限分类表达比过去更自然。更合适的做法是借鉴分类稳定、诊断链、边界集中输出的思想，而非照搬返回值式载体。
...
第三步，边界层交给 `@ControllerAdvice` 统一输出。`@ExceptionHandler` 按域注册：`OrderError` 的 handler 处理所有业务语义域的错误，`RepositoryError` 如果没有被上层转换则在边界作为 500 兜底。
...
public class ErrorPolicy {
    ...
    public ResponseEntity<ErrorBody> handleOrder(OrderError e) {
        ...
        return ResponseEntity
            .status(statusOf(e.identity()))          // 契约通道：基于身份路由
...
| 契约通道 | reason 枚举变体 + identity 字符串 | 子类覆写的 `identity()` / `category()` / `retryable()` |
