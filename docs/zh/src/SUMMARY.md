# Summary

- [中文首页](README.md)

## 用户文档

- [为什么需要 orion-error](user/why-orion-error.md)
- [使用教程](user/tutorial.md)
- [OrionError 与稳定身份](user/reason-identity-guide.md)
- [协议契约](user/protocol-contract.md)
- [Report / Exposure 边界](user/report-exposure-boundary.md)
- [日志说明](user/LOGGING.md)
- [生态方案对比](user/ecosystem-comparison.md)
- [与 thiserror 的关系](user/thiserror-comparison.md)
- [大型工程错误治理宣言](user/manifesto.md)
- [大型工程错误治理宣言-blog](user/manifesto-blog.md)
- [错误治理与 AI 编程](user/errors-and-ai.md)
- [设计约束](user/design-constraints.md)

## 开发文档

- [设计架构](dev/architecture.md)
- [API Contract](dev/api-contract.md)
- [兼容与迁移](dev/compat-migration.md)
- [Public Surface Grading](dev/public-surface-grading.md)
- [Release Checklist](dev/release-checklist.md)
- [StructError Allocation](dev/perf/structerror-allocation.md)
- [StructError Source Debug](dev/perf/structerror-source-debug.md)
